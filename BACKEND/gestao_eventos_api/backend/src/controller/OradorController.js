// src/controllers/OradorController.js
const Model = require('../models/OradorModel');

async function getAllOradores(req, res) {
  try {
...
async function getOradorById(req, res) {
  try {
...
async function createOrador(req, res) {
  try {
    const id = await Model.create(req.body);
    res.status(201).json({ ID_Orador: id });
  } catch (e) {
    res.status(400).json({ error: e.message });
  }
}

async function updateOrador(req, res) {
  try {
...
async function deleteOrador(req, res) {
  try {
