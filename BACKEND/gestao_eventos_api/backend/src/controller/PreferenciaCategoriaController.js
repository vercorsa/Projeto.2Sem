const Model = require('../models/PreferenciaCategoriaModel');

async function getAllPreferencias(req, res) {
  try {
...
async function getPreferenciaById(req, res) {
  try {
...
async function createPreferencia(req, res) {
  try {
    const id = await Model.create(req.body);
    res.status(201).json({ ID_Preferencia: id });
  } catch (e) {
    res.status(400).json({ error: e.message });
  }
}

async function updatePreferencia(req, res) {
  try {
...
async function deletePreferencia(req, res) {
  try {
